    ...
    if hands:
        ...
        if sum(fingers) == 1:
            ...
            while True:
                ...
                # print(success)
                frame = cv2.flip(frame, 1)  # flip on ya axis (1 is for y)
                # frame = cv2.GaussianBlur(frame1, (7,7), 0)          # less blurring the the average but more natural
                ...
                if hands:
                    for hand in hands:
                        ...
                        for id, landmark in enumerate(landmarks):
                            ...
                            # if id==8 and classification=="Left":
                            #     cv2.circle(img=frame,center=(x,y),radius=10,color=(0,255,255),thickness=-1)
                            #     index_x_l=screen_width*1.01/frame_width*x
                            #     index_y_l=screen_height*1.01/frame_height*y
                            #     if abs(thumb_y_l-index_y_l) < 50:
                            #         print('Closing Windows')
                            #         pyautogui.press('q')

                cv2.imshow('Virtual Media Player', frame)

                if cv2.waitKey(15) & 0xFF == ord('q'):
                    pyautogui.sleep(
                        5)  # says if the frame is there for 15 sec or if d is pressed break out of the video
                    break
            ...
            while (True):
                ...
                if hands:
                    ...
                    xVal = int(np.interp(lmlist[8][0], [(width // 2) + 150, w], [50, width]))
